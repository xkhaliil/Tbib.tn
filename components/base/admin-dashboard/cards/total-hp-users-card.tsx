...
import {
  getHealthcareProvidersCount,
  totalHealthcareProvidersYearlyWithIncrease,
} from "@/actions/healthcare-provider";

export async function TotalHealthCareProvidersUsersCard() {
  const totalHealthcareProvidersInThisYear =
    await totalHealthcareProvidersYearlyWithIncrease();
  const totalHealthcareProvidersCount = await getHealthcareProvidersCount();
