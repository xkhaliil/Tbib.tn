...
interface AdditionalImagesDropzoneProps {
  value: string[] | undefined;
  onChange: (url?: string[]) => void;
}

export function AdditionalImagesDropzone({
  value,
  onChange,
}: AdditionalImagesDropzoneProps) {
  const [uploadProgress, setUploadProgress] = React.useState<number>(0);

  const { startUpload, isUploading } = useUploadThing("additionalImages", {
  ...
  return (
    ...
    >
      {({ getRootProps, getInputProps, acceptedFiles }) => (
        <div
          {...getRootProps()}
          className="h-96 rounded-lg border border-dashed border-gray-300"
        >
