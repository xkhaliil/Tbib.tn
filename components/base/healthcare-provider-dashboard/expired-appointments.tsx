...
interface ExpiredAppointmentsBarProps {
  appointments: AppointmentsWithPatient | undefined;
}

export function ExpiredAppointments({
  appointments,
}: ExpiredAppointmentsBarProps) {
  return (
    <>
      {appointments?.length ? (
        appointments?.map((appointment) => (
          <div
            key={appointment?.id}
            className="flex items-center border-b border-l-[6px] border-l-rose-500/35 p-4"
          >
