...
export async function ChatSidebarItem({ conversation }: ChatSidebarItemProps) {
  ...
  return (
    <Link href={`/patient/dashboard/messages/conversations/${conversation.id}`}>
      <div className="relative flex w-full cursor-pointer items-center space-x-3 rounded-xl px-2.5 py-4 transition hover:bg-muted">
        <UserAvatar
          selectedUser={conversation.healthCareProvider.user}
          className="h-11 w-11"
        />
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <p className="text-sm font-medium text-foreground">
                  Dr. {conversation.healthCareProvider.user.name}
                </p>
                <p
                  className={cn(
